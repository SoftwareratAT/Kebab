...
/**
 * Modified MCAUtil. Original net.querz.mca.NBTUtil
 */
public final class KebabMCAUtil {
    private KebabMCAUtil() {}

    public static KebabMCAFile read(String file) throws IOException {
        return read(new File(file), LoadFlags.ALL_DATA);
    }

    public static KebabMCAFile read(File file) throws IOException {
        return read(file, LoadFlags.ALL_DATA);
    }

    public static KebabMCAFile read(String file, long loadFlags) throws IOException {
        return read(new File(file), loadFlags);
    }

    public static KebabMCAFile read(File file, long loadFlags) throws IOException {
        KebabMCAFile mcaFile = newMCAFile(file);
    ...
    public static int write(KebabMCAFile mcaFile, String file) throws IOException {
        return write(mcaFile, new File(file), false);
    }

    public static int write(KebabMCAFile mcaFile, File file) throws IOException {
        return write(mcaFile, file, false);
    }

    public static int write(KebabMCAFile mcaFile, String file, boolean changeLastUpdate) throws IOException {
        return write(mcaFile, new File(file), changeLastUpdate);
    }

    public static int write(KebabMCAFile mcaFile, File file, boolean changeLastUpdate) throws IOException {
        File to = file;
        if (file.exists()) {
    ...
    public static KebabMCAFile newMCAFile(File file) {
        Matcher m = mcaFilePattern.matcher(file.getName());
        if (m.find()) {
            return new KebabMCAFile(Integer.parseInt(m.group("regionX")), Integer.parseInt(m.group("regionZ")));
        }
