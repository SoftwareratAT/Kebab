...
public final class KebabPlayerTick extends Thread {
    private static final Logger LOGGER = LoggerFactory.getLogger(KebabPlayerTick.class);

    private final KebabServer server;
    private final short tickingInterval;
    private final AtomicLong tick;
    KebabPlayerTick(KebabServer server) {
        short tps = Kebab.getConfig().getPlayerTps();
        this.server = server;
    ...
    public void run() {
        while (server.isRunning()) {
            ...
            try {
                updatePlayers(server.getUnsafePlayers());
            } catch (IOException exception) {
                LOGGER.error("Cannot run update for each player! Tick: " + getCurrentTick(), exception);
            }
    ...
    private void updatePlayers(Collection<KebabPlayer> players) throws IOException {
        for (KebabPlayer player : players) {
